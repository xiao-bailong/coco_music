 ...
 *
 */
public class MajortypeCategoryServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if("toMajortypeCategoryListView".equals(method)){
			request.getRequestDispatcher("/WEB-INF/views/majortype_category.jsp").forward(request, response);
			return;
		}
		if("MajortypeCategoryList".equals(method)){
	...
	private void deleteBookCategory(HttpServletRequest request,
		...
		if(!majortypeCategoryDao.delete(idArr)){
			ret.put("type", "error");
	...
	private void editBookCategory(HttpServletRequest request,
		...
		majortypeCategory.setName(name);
		majortypeCategory.setId(id);
		if(!majortypeCategoryDao.update(majortypeCategory)){
			ret.put("type", "error");
	...
	private void addBookCategory(HttpServletRequest request,
		...
		MajortypeCategory majortypeCategory = new MajortypeCategory();
		majortypeCategory.setName(name);
		if(!majortypeCategoryDao.add(majortypeCategory)){
			ret.put("type", "error");
	...
	private void getBookCategoryList(HttpServletRequest request,
		...
		Page<MajortypeCategory> page = new Page<MajortypeCategory>(pageNumber, pageSize);
