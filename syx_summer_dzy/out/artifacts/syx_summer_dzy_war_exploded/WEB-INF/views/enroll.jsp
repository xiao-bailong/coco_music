...
<script type="text/javascript">
	...
	$(function() {	
	    ...
	    $('#dataList').datagrid({ 
	        ...
	        fit: true,//自动大小 
	        method: "post",
	        url:"EnrollServlet?method=EnrollList&t="+new Date().getTime(),
	        idField:'id', 
        	...
        	if(selectLength == 0){
            	...
            	$.messager.confirm("消息提醒", "删除高校分类前请确保该高校分类没有借阅信息，否则可能会导致借阅功能异常，确认继续？", function(r){
            		if(r){
            			$.ajax({
							type: "post",
							url: "EnrollServlet?method=DeleteBookCategory",
							data: {ids: ids},
							dataType:'json',
	    	...
	    	buttons: [
	    		{
					...
					handler:function(){
						...
						if(!validate){
							...
							$.ajax({
								type: "post",
								url: "EnrollServlet?method=AddBookCategory",
	    	...
	    	buttons: [
	    		{
					...
					handler:function(){
						...
						if(!validate){
							...
							$.ajax({
								type: "post",
								url: "EnrollServlet?method=EditBookCategory&t="+new Date().getTime(),
